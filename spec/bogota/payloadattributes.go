...
// PayloadAttributes is the SSZ container `PayloadAttributesV5`: V4 extended
// with `inclusionListTransactions` (EIP-7805). The two SSZ-level dimension
// caps on the inclusion-list field both use MAX_BYTES_PER_INCLUSION_LIST
// because the spec only defines that one bound; a one-byte-per-tx minimum
// is a safe upper bound on the count.
type PayloadAttributes struct {
	...
	InclusionListTransactions []paris.Transaction    `dynssz-max:"MAX_BYTES_PER_INCLUSION_LIST,MAX_BYTES_PER_INCLUSION_LIST" ssz-max:"8192,8192" ssz-size:"?,?" json:"inclusionListTransactions"`
}
...
type payloadAttributesJSON struct {
	...
	InclusionListTransactions []paris.Transaction    `json:"inclusionListTransactions"`
}
...
func (p *PayloadAttributes) MarshalJSON() ([]byte, error) {
	...
	return json.Marshal(&payloadAttributesJSON{
		...
		InclusionListTransactions: p.InclusionListTransactions,
	})
}
...
func (p *PayloadAttributes) UnmarshalJSON(input []byte) error {
	...
	if data.InclusionListTransactions == nil {
		return errors.New("PayloadAttributes: inclusionListTransactions missing")
	}
	...
	p.InclusionListTransactions = data.InclusionListTransactions
